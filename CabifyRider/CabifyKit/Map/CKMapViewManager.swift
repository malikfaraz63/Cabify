...
class CKMapViewManager: NSObject, MKMapViewDelegate {
    ...
    func clearMapView(removesAnnotations: Bool = true) {
        guard let mapView = mapView else { return }
        mapView.removeOverlays(mapView.overlays)
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        } else if annotation is CKDriverAnnotation {
            guard let driverAnnotation = annotation as? CKDriverAnnotation else { fatalError() }
            
            return driverAnnotation.getView()
