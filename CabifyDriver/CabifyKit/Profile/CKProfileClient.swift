...
class CKProfileClient {
    ...
    public func tryFetchSession(forDriverId driverId: String, completion: @escaping SessionFetchCompletion) {
        db
            .collection("requests")
            .whereField("driverId", isEqualTo: driverId)
            .whereField("status", isEqualTo: CKRequestStatus.active.rawValue)
            .getDocuments(source: .server) { querySnapshot, error in
                if let querySnapshot = querySnapshot {
                    if querySnapshot.isEmpty {
                        self.tryRideFetch(forDriverId: driverId, completion: completion)
                        return
                    }
    ...
    private func tryRideFetch(forDriverId driverId: String, completion: @escaping SessionFetchCompletion) {
        db
            .collection("rides")
            .whereField("driverId", isEqualTo: driverId)
